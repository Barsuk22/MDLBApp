...
fun HabitsScreen(navController: NavController) {
    ...
    LaunchedEffect(Unit) {
        ...
        val me = Firebase.firestore.collection("users").document(mommyUid).get().await()
        babyUid = me.getString("pairedWith")
        babyUid?.let { bUid ->
            val u = Firebase.firestore.collection("users").document(bUid).get().await()
            babyZone = runCatching { ZoneId.of(u.getString("timezone") ?: "") }.getOrNull()
    ...
    val today = LocalDate.now(babyZone ?: ZoneId.systemDefault())
                ...
                LazyColumn(
                    ...
                    groupedHabits.forEach { (date, habitsForDate) ->
                        ...
                        items(habitsForDate) { habit ->
                            MommyHabitCard(habit, navController, zone = babyZone ?: ZoneId.systemDefault())
                        }
                    }
